...
#define KEY_LENGTH 31 // Can be anything from 1 to 13
...
unsigned char calculateKey(const size_t repeat, const unsigned char *groupedStream) {
    ...
    for (unsigned char b = 0x00; b < 0xFF; ++b)
    {
        ...
        for (size_t j = 0; j < repeat; ++j)
        {
            ...
            if( ((0x21 <= shiftedStream[j] && shiftedStream[j] <= 0x40 )) ||
                ((0x5B <= shiftedStream[j] && shiftedStream[j] <= 0x60 )) )
            {
                validGuess = 0;
                break;
            }
...
size_t readFile(unsigned char **pOut)
{
    ...
    FILE *cipherFile = fopen("ctext_maryland.txt", "r");
...
int crackVigenere()
{
    ...
    */
    ...
    for (size_t count = 0; count < size; ++count)
    {
        ...
        if(count !=0 && count%keyLength == 0) printf("\n");
    }
    printf("\n");
